<?php require_once('header.php'); ?>

<div class="page-banner">
    <div class="inner">
        <h1 style="color: black;">About Us</h1>
    </div>
</div>

<div class="page">
    <div class="container">
        <div style="display: flex; justify-content: space-between; align-items: center; width: 100%; margin-bottom:75px;">
            <div>
                <iframe style="width: 500px; height:300px;" src="https://www.youtube.com/embed/sGho_JThB5M?autoplay=1" title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"></iframe>
            </div>
            <div style="width: 50%; font-size: 2rem; padding: 10px; margin: 2rem; display: inline;">
                <p>
                    Welcome to Redstar, where we're passionate about revolutionizing your cycling experience with our cutting-edge electric bikes. We believe that biking should be accessible to everyone, and our innovative eBikes are designed to provide a thrilling and eco-friendly alternative to traditional cycling.
                </p>
            </div>
        </div>
        <div style="display: flex; justify-content: space-between; align-items: center; width: 100%; margin-bottom:75px;">
            <div style="width: 50%; font-size: 2rem; padding: 10px; margin: 2rem; display: inline;">
                <p>
                    At Redstar, our mission is to create high-quality electric bikes that combine style, performance, and sustainability. We strive to empower riders of all skill levels to enjoy the benefits of cycling while reducing their carbon footprint. We're committed to developing innovative solutions that enhance your daily commute, outdoor adventures, and everything in between.
                </p>
            </div>
            <div>
                <img style="width: 425px; height:300px;" src="assets/img/about.jpg" alt="">
            </div>
        </div>
        <div style="display: flex; justify-content: space-between; align-items: center; width: 100%; margin-bottom:75px;">
            <div>
                <img style="width: 415px; height:300px;" src="assets/img/RSimage01.jpg" alt="">
            </div>
            <div style="width: 50%; font-size: 2rem; padding: 10px; margin: 2rem; display: inline;">
                <p>
                    At Redstar, we're dedicated to providing exceptional customer service and ensuring your satisfaction throughout your eBike journey. Our knowledgeable team is ready to assist you with any inquiries, from pre-purchase guidance to after-sales support. We're committed to building lasting relationships with our customers and helping you make the most of your RedStar eBike experience.
                    <br>
                    <br>
                    Thank you for choosing Redstar. Join us on our mission to transform the way we ride and embrace a cleaner, more exciting future.
                </p>
            </div>

        </div>
    </div>
</div>

<?php require_once('footer.php'); ?>